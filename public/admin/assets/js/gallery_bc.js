$(document).ready(function(){
    ...
    $('#openNewGalleryModal').click(function(){
        ...
        $('#img_alt').val('');
        $('#gall_img').empty();
        var htm = '<div class="form-group"><label>Gallery Image</label><input type="file" name="gal_image" id="gal_image" class="form-control"><span id="gal_image_error" style="color:red"></span></div>';
        $('#gall_img').html(htm);
        $.ajax({
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            if(data.status == 'success')
            {
                ...
                $('#insert_gallery').hide();
                $('#gall_img').empty();
                var ht = '<div class="form-group col-md-8"><label>Gallery Image</label><input type="file" name="gal_image" id="gal_image" class="form-control"></div>';
                    ht += '<div class="form-group col-md-4"><label>Image Preview</label><img src="/upload/gallery/'+data.galleryData['image_name']+'" width="50"></div>';
                $('#gall_img').html(ht);
